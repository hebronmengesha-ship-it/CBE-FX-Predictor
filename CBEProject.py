...
st.set_page_config(
    page_title="The CBE Corporate FX Risk Simulator",
    layout="wide",
...
<style>
    ...
    /* Settlement Tool Layout Fix (Number input text + border) */
    ...
    /* Ensure BaseWeb input styles don't override text color */
...
# --- 3. DASHBOARD HEADER ---
st.markdown("### **The CBE Corporate FX Risk Simulator**")
st.markdown(
    f"<p style='font-size: 13px; font-weight: 700; margin-top:-15px;'>SYSTEM CLOCK: {date.today().strftime('%Y-%m-%d')} | ARCHITECTURE: HYPER-ENSEMBLE (24 CONFIGS)</p>",
    unsafe_allow_html=True
)
st.markdown("---")
...
def hyper_ensemble_arena(df, horizon):
    ...
    # ARIMA 12-Pack
    configs = [
        (1,1,0), (2,1,0), (5,1,0), (0,1,1), (1,1,1), (2,1,2),
        (5,1,2), (1,2,1), (0,2,1), (3,1,1), (4,1,0), (2,1,1)
    ]
    for cfg in configs:
        try:
            res = ARIMA(train_vals, order=cfg).fit().forecast(steps=60)
            scores[f"ARIMA{cfg}"] = float(np.sqrt(mean_squared_error(test_vals, res)))
        except:
            pass
    ...
    for d in [5, 10, 20, None]:
        for e in [50, 100]:
            try:
                ...
                scores[f"RF_D{d}_E{e}"] = float(np.sqrt(mean_squared_error(test_vals, m.predict(test_x))))
            except:
                pass
...
try:
    ...
    if not df_raw.empty:
        ...
        ax.fill_between(
            f_dates, low_b, high_b,
            color="#2962FF", alpha=0.08,
            label=f"90% RISK CORRIDOR ({model_count} MODELS)"
        )
        ...
        with c2:
            st.markdown(
                f'<div class="settlement-display">EXECUTION RATE [{f_dates[t_day-1].strftime("%Y-%m-%d")}]: {final_p[t_day-1]:.2f} ETB</div>',
                unsafe_allow_html=True
            )
